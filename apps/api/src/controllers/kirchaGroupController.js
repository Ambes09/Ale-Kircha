...
export class KirchaGroupController {
    async getAllGroups(request, reply) {
        const { status, kirchaTypeId } = request.query;
        const where = {};
        if (status)
            where.status = status;
        if (kirchaTypeId)
            where.kirchaTypeId = kirchaTypeId;
    ...
    async getAvailableGroups(request, reply) {
        const groups = await prisma.kirchaGroup.findMany({
            where: {
                status: 'OPEN',
                totalCapacity: { gt: 0 },
    ...
    async updateGroupStatus(request, reply) {
        const { id } = request.params;
    ...
    async joinGroup(request, reply) {
        const { id } = request.params;
        const { customerId, quantity, portionType } = request.body;
    ...
    async migrateUser(request, reply) {
        const { fromGroupId, toGroupId, customerId, quantity } = request.body;
        return prisma.$transaction(async (tx) => {
            ...
            if (!newGroup)
                throw new NotFoundError('Destination group');
